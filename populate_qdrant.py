...
class QdrantPopulator:
    ...
    def __init__(self):
        ...
        # Qdrant client (in-process, disk-persisted)
        self.qdrant_client = QdrantClient(path=QDRANT_DATA_PATH)

        # Text embedder
        self.embedder = TextEmbedder()

        # Groq client for generating descriptions
        groq_api_key = os.getenv('GROQ_API_KEY')
        if groq_api_key:
            self.groq_client = Groq(api_key=groq_api_key)
        else:
            self.groq_client = None
            print("⚠ GROQ_API_KEY not found - text generation disabled")
...
def main():
    ...
    try:
        ...
        print(f"\n✗ Error: {e}")
        print("\nMake sure:")
        print("  1. qdrant-client is installed (pip install qdrant-client)")
        print("  2. Groq API key is set in .env file as GROQ_API_KEY")
