...
class VectorDB:
    ...
    def __init__(self, qdrant_path: str = "./qdrant_data"):
        ...
        Args:
            qdrant_path: Path to Qdrant on-disk storage
        """
        self.client = QdrantClient(path=qdrant_path)
        self.face_collection = "face_embeddings"
        self.text_collection = "text_embeddings"
        
        print(f"✓ Connected to Qdrant (disk: {qdrant_path})")
    ...
    def setup_text_collection(self, vector_size: int = 768, recreate: bool = False):
        """
        Create collection for text embeddings.
        
        Args:
            vector_size: Dimension of text embeddings (default: 768 for all-mpnet-base-v2)
            recreate: If True, delete existing collection and recreate
        """
        try:
            collections = self.client.get_collections().collections
    ...
    def setup_all_collections(self, recreate: bool = False):
        ...
        # Text embeddings: 768 dimensions (Sentence Transformers all-mpnet-base-v2)
        self.setup_text_collection(vector_size=768, recreate=recreate)
...
if __name__ == "__main__":
    ...
    Uses in-process disk-persisted Qdrant (no server required).
    """
    ...
    try:
        # Initialize vector database (disk-persisted, no server needed)
        vector_db = VectorDB(qdrant_path="./qdrant_data")
        ...
        print("  - text_embeddings: 768 dimensions (for Sentence Transformers)")
        
    except Exception as e:
        print(f"\n✗ Error: {e}")
        print("\nMake sure qdrant-client is installed:")
